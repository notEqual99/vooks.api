...
<div class="home container">
    ...
    <div class="input-group search-box mb-3">
        <form action="{{route('search.vooks')}}" method="POST" role="search" class="form-inline">
            {{ csrf_field() }}
            <input type="search" class="search-input form-control" name="q" placeholder="Searching Books ..." required="" aria-label="Recipient's username" aria-describedby="basic-addon2">
    ...
    @if($books->count() > 0)
        <h1 class="main-title">
            {{ Request::is('search/vooks') ? 'Results by search' : 'Book Collections' }}
            <span></span>
        </h1>
        <br/>
            ...
            <div class="row">
                <div class="image-panel col-12 col-sm-3 pt-3">
                    <div class="thumbnail-container reviews-loading">
                        <a href="{{route('vook.show', $book->id)}}" class="thumbnail product-thumbnail" rel="noopener noreferrer">
    ...
    @else
        <h1 class="main-title">Sorry! no books found.</h1>
    @endif
</div>
@endsection